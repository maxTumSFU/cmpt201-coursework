...
void groupByKey(IntermediateInput *input, Output *output, int *result_count) {
  ...
  // search through all doubled values
  for (int i = 0; i < *result_count; i++) {
    // doubled value already stored
    if (output[i].doubled_value == input->doubled_value) {
      // store each line number for each type of doubled value
      output[i].line_numbers[output[i].count] = input->line_number;
      output[i].count++; // increase count to move up index in array
      return;            // so no new entries created
    }
  }
  // need to create entry for new doubled value
...
void reduce(Output *output) {
  ...
  for (int i = 0; i < output->count; i++) {
    ...
    // print commas if not last element
    if (i < output->count - 1) {
      printf(", ");
    }
  }
