...
2. Is it UDP or TCP? How do you know?
    TCP - line 49 has client connecting to server. TCP feature
3. The client is going to send some data to the server. Where does it get this data from? How can
you tell in the code? line 54 --> read() data from terminal. (STDIN_FILENO)
4. How does the client program end? How can you tell that in the code?
    ends when there is an error, or when 1 or less bytes are read (only \n entered as input)
*/
